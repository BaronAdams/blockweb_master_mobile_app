 ...
 * structure — rather than a hand-rolled native UI, so the two surfaces stay
 * visually identical without duplicating a whole design in Kotlin. All
 * strings come from a JSON bundle JS pushes via
 * Blocker.setBlockScreenStrings() (see lib/blockedScreenStrings.ts); native
 * only ever substitutes the literal "{{value}}" token, it has no i18n.
 */
class BlockOverlay(private val service: AccessibilityService) {
  ...
  private fun openManageRules() {
    try {
      val intent = Intent(Intent.ACTION_VIEW, Uri.parse("$APP_SCHEME://blocklists")).apply {
        addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
      }
      service.startActivity(intent)
    } catch (e: Exception) {
      try {
        val fallback = service.packageManager.getLaunchIntentForPackage(service.packageName)
        fallback?.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        fallback?.let { service.startActivity(it) }
      } catch (e2: Exception) {}
    }
  }
  ...
  // ---- Strings bundle (see lib/blockedScreenStrings.ts) ------------------
  ...
  // ---- Per-reason colors + icons, matching App.tsx's PROFILE_CONTENT/getContent() ----

  /** (icon color, glow color) — the only two things the extension actually
   *  tints by reason; the detail/reason icon boxes are always neutral gray. */
  ...
  // ---- HTML document, mirroring App.tsx + App.css exactly -----------------
...
<body>
  ...
  companion object {
    ...
    private const val APP_SCHEME = "blockweb-master-mobile-app"
  }
}
